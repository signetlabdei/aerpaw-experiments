...
echo "Experiment will run with model"$mdl".mgn application, using the "$vehicle"."
...
./Radio/startRadio.sh
./Traffic/startTrafficCLArguments.sh $mdl

if [[ $vehicle == "drone" ]] 
then
  echo "Running drone script."
  ./Vehicle/startDrone.sh
elif [[ $vehicle == "rover" ]] 
then
  echo "Running rover script."
  ./Vehicle/startRover.sh
elif [[ $vehicle == "gpsLogger" ]] 
then
  echo "Running GPS logger script."
  ./Vehicle/startGPSLogger.sh
else
  echo "Warning: vehicle not recognized, abort mission."
  ./stopAll.sh
fi
